...
class RTree
{
    ...
    public:
		...
		void Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::vector<DATATYPE>& res);
    ...
    protected:
		...
		bool Search(Node* a_node, Rect* a_rect, std::vector<DATATYPE>& res);
		void Reset();
...
int RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], t_resultCallback a_resultCallback, void* a_context)
{
	...
	RTREE_TEMPLATE
void RTREE_QUAL::Search(const ELEMTYPE a_min[NUMDIMS], const ELEMTYPE a_max[NUMDIMS], std::vector<DATATYPE>& res)
{
#ifdef _DEBUG
    for(int index=0; index<NUMDIMS; ++index)
    {
...
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, int& a_foundCount, t_resultCallback a_resultCallback, void* a_context)
{
	...
	RTREE_TEMPLATE
bool RTREE_QUAL::Search(Node* a_node, Rect* a_rect, std::vector<DATATYPE>& res)
{
	ASSERT(a_node);
    ASSERT(a_node->m_level >= 0);
    ASSERT(a_rect);
    ...
    if(a_node->IsInternalNode())
    {
        ...
        for(int index=0; index < a_node->m_count; ++index)
        {
            if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
            {
                Search(a_node->m_branch[index].m_child, a_rect, res);
            }
        }
    }
    else
    {
        ...
        for(int index=0; index < a_node->m_count; ++index)
        {
            if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
            {
				res.push_back(a_node->m_branch[index].m_data);
            }
        }
    }
